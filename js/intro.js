...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const player = pre ? pre.querySelector("lottie-player") : null;
  if (!pre || !page || !player) return;
  ...
  if (visitedRecently() || prefersReduce) {
    try { player.stop && player.stop(); } catch {}
    hidePreloader();
    return;
  }

  let safetyTimeoutId = null;
  let minTimeoutId = null;
  ...
  const startIntro = () => {
    try { player.seek && player.seek(0); } catch {}
